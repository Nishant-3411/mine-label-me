...
class Shape(object):
    ...
    def paint(self, painter):
        ...
        if self.points:
            ...
            # print("LINE COLOR:", self.line_color, self.select_line_color)
            # print("COLOR:", color)
            ...
            if len(self.points)>2:
                self.drawSpline(line_path)
            
            # if self.isClosed() and not self.pathClose:
            if self.isClosed():
                if self.addedPoint:
    ...
    def drawVertex(self, path, i):
        ...
        if shape == self.P_SQUARE:
            ...
            # print("PX:", px, py)
            if type(point) == 'dict':
                path.addEllipse(px, py, d / 2.0, d / 2.0)
            else:
                path.addEllipse(point.x(), point.y(), d / 2.0, d / 2.0)
        else:
            assert False, "unsupported vertex shape"


    def drawSpline(self, path):
        
            
        dEllipse = self.point_size / self.scale
        n=len(self.points)
        npa = [[self.points[i].x(), self.points[i].y()] for i in range(len(self.points))]
        ...
        for t in np.arange(0, 1, 0.01):
            for i in range(n-1):
                ...
                # Y = Y.astype(int)
                px, py = Y[0].item(), Y[1].item()
                path.addEllipse(px, py, dEllipse / 8.0, dEllipse / 8.0)
    ...
    def nearestVertex(self, point, epsilon):
        ...
        min_i = None
        # print("POINTS:", len(self.points), self.label)
        for i, p in enumerate(self.points):
